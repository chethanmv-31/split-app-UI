...
export function CreateGroupForm({ onCancel, onSuccess }: CreateGroupFormProps) {
    ...
    const handleInviteUser = async (contactName?: string, contactPhone?: string) => {
        ...
        if (phone) {
            ...
            if (existingUser) {
                Alert.alert('Already Added', `This contact already exists as "${existingUser.name}".`);
                setInviting(false);
                return;
            }
        }
    ...
    const handleCreateGroup = async () => {
        ...
        setCreating(true);
        const payload: { name: string; createdBy: string; members: string[]; invitedUsers?: Array<{ name: string; mobile?: string }> } = {
            name: finalGroupName,
